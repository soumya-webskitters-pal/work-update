...
/*--NOTE: number of images in the video sequence*/
...
jQuery(function ($) {
    ...
    if ($("#about").length) {
        ...
        const frame_duration = 600, frame_scrub = 2;
    ...
    if ($(".nav_anm").length) {
        gsap.set(".nav_anm path", {
            opacity: 1,
        })
        $(".nav_anm .trb_g").each(function (index) {
            $(this).attr("mask", "url(#menucstMask" + index + ")");
            $(".nav_anm mask:eq(0)").clone().attr("id", "menucstMask" + index).addClass("menu_trib_mask").prependTo(".nav_anm");
        });
        gsap.set(".menu_trib_mask rect", {
            yPercent: 100,
        });
    }
    ...
    if ($("#menu").length) {
        ...
        // function bannerVideoAnim() {
        //     /// page banner animation
        //     gsap.to($(".sec_banner video"), 1, {
        //         opacity: 1,
        //         stagger: 0.1,
        //     });
        //     $(".sec_banner video").each(function () {
        //         this.play();
        //         $(this).attr("loop", "true");
