...
MODEL = function () {
    ...
    MODEL.prototype.SceneLoad = function () {
        ...
        myFullpage = new fullpage('#fullpage', {
            anchors: ['banner_page', 'solutions_page', 'market_page', 'resources_page', 'company_page', 'contact_page'],
            scrollOverflow: true,
            autoScrolling: true,
            menu: '#mainmenu',
            css3: true,
            ...
            afterLoad: function (anchorLink, index, direction, origin) {
                if (index.item.classList.contains("darkTheme")) {
                    document.body.classList.add("dark");
                    document.body.classList.remove("light");
                }
                if (index.item.classList.contains("lightTheme")) {
                ...
                if (index.item.children[0].children[0].classList.contains("section-inner")) {
                    if (!index.item.children[0].classList.contains("fp-auto-height")) {
    ...
    MODEL.prototype.ClickHandle = function (event) {
        console.log(event);
        $(navigation).find('li[data-menuanchor]').click(function () {
            var _page_pos = $(navigation).find('li[data-menuanchor]').index(this) + 1;
            fullpage_api.moveTo(_page_pos, _page_pos);
        });
        ...
        function openNav() {
            ...
            $.each($(submenuEl), function (index, element) {
                ...
                if (subMenu.length != 0) {
                    ...
                    tl.staggerTo(subMenu, 0.2, { y: 0, display: "block", height: "auto", autoAlpha: 1, }, 0.1, 0.1);
                    element.subMenuAnimation = tl;
                    $(element).hover(menuItemOver, menuItemOut);
                }
            });
            function menuItemOver(e) {
                TweenMax.set($(this).find(".sub-menu"), { display: "block", height: "auto" });
