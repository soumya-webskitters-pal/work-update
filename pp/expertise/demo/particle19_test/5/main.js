...
const smoothScrollAmount = 10;
var smoothScrollFlag = 0;
...
MODEL = function () {
    ...
    MODEL.prototype.Animate = function () {
        ...
        if (smoothScroll) {
            ...
            if (scrollCount >= 0) {
                if (whellUp) {
                    if (scrollStop <= 0) {
                        var scrollAnimStart = setInterval(() => {
                            // console.log(velocity);
                            progress += velocity;
                            world.position.y += nodePositionFocusOffset;
                        }, smoothScrollAmount / 2);
                        setTimeout(() => {
                            clearInterval(scrollAnimStart);
                            smoothScroll = false;
                        }, smoothScrollAmount);
                    }
                } else {
                    var scrollAnimStop = setInterval(() => {
                        progress -= velocity;
                        if (progress < 0) {
                            progress = 0;
                        }
                ...
                var NowConvtProgress = progress.toFixed(3);
                console.log(NowConvtProgress);
                if (NowConvtProgress >= 0 && progress < progressStep[0] - 0.05) {
                    focus_node = 0;
                }
                if (NowConvtProgress >= progressStep[0] && NowConvtProgress < progressStep[0] + 0.05) {
                    focus_node = 1;
                    smoothScroll = false
                } else if (NowConvtProgress >= progressStep[1] && NowConvtProgress < progressStep[1] + 0.05) {
                    focus_node = 2;
                    smoothScroll = false
                } else if (NowConvtProgress >= progressStep[2] && NowConvtProgress < progressStep[2] + 0.05) {
                    focus_node = 3;
                    smoothScroll = false
                } else if (NowConvtProgress >= progressStep[3] && NowConvtProgress < progressStep[3] + 0.05) {
                    focus_node = 4;
                    smoothScroll = false
                } else if (NowConvtProgress >= progressStep[4] && NowConvtProgress < progressStep[4] + 0.05) {
                    focus_node = 5;
                    scrollStop = 1;
                    smoothScroll = false
                }
        ...
        position.y += 0.5;//push camera to bottom
        world.position.copy(position);
        tangent.copy(curve.getTangentAt(progress));

        velocity = Math.max(0.0008, Math.min(0.002, velocity));
    ...
    MODEL.prototype.nodeProgress = function () {
