...
MODEL = function () {
    ...
    MODEL.prototype.Init = function () {
        ...
        DirectionalLight = new THREE.DirectionalLight("#ffffff", 1.7);
        ...
        // var instagram_pass = new THREE.ShaderPass(THREE.InstagramFilter);
        // instagram_pass.renderToScreen = true;
        // instagram_pass.uniforms.vignette.value = 0.5;
        // instagram_pass.uniforms.exposure.value = 0.8;
        // instagram_pass.uniforms.color.value = new THREE.Color("#ffffff");
        // instagram_pass.uniforms.center.value = this.camera.position;
        // composer.addPass(instagram_pass);
    ...
    MODEL.prototype.Render = function () {
        ...
        // composer.render(0.5); //parameter must be set with render
    },
    ...
    MODEL.prototype.Animate = function () {
        ...
        for (let index = 0; index < _solidGroup.children.length; index++) {
            ...
            //_solidGroup.children[index].rotation.set(trRotateTime, trRotateTime, trRotateTime);
            // var MMax = 1;
            // var MMin = -2;
            // TweenMax.to(_solidGroup.children[index].position, 25, {
            //     x: _solidGroup.children[index].position.x + (Math.random() * (MMax - (-MMin) + 1) + (-MMin)),
            //     z: _solidGroup.children[index].position.z + (Math.random() * (MMax - (-MMin) + 1) + (-MMin))
            // });
