...
MODEL = function () {
    ...
    MODEL.prototype.SceneLoad = function () {
        ...
        // torus
        var shape_torus = new THREE.Mesh(new THREE.TorusBufferGeometry(100, 60, 16, 40), shapeMaterial);
    ...
    MODEL.prototype.Animate = function () {
        ...
        if (smoothScroll) {
            ...
            if (scrollCount >= 0) {
                ...
                if (progress >= 0 && progress < progressStep[0] - 0.01) {
                    focus_node = 0;
                }
                if (progress >= progressStep[0] && progress < progressStep[0] + 0.01) {
                    focus_node = 1;
                    smoothScroll = false
                } else if (progress >= progressStep[1] && progress < progressStep[1] + 0.01) {
                    focus_node = 2;
                    smoothScroll = false
                } else if (progress >= progressStep[2] && progress < progressStep[2] + 0.01) {
                    focus_node = 3;
                    smoothScroll = false
                } else if (progress >= progressStep[3] && progress < progressStep[3] + 0.01) {
                    focus_node = 4;
                    smoothScroll = false
                } else if (progress >= progressStep[4] && progress < progressStep[4] + 0.01) {
                    focus_node = 5;
                    scrollStop = 1;
                    smoothScroll = false
                }
    ...
    MODEL.prototype.openModal = function (node_data) {
        ...
        if (node_data.content.name !== null) {
            ...
            _dataBox.style.maxHeight = parseFloat(NowDataBoxHeight) + "px";
            ...
            var setHeight = NowDataBoxHeight -
                (parseFloat(getComputedStyle(_dataBox).paddingTop) +
                    ...
                    _header.parentNode.clientHeight +
                    parseFloat(getComputedStyle(_header.parentNode).marginBottom));
            _description.parentNode.style.maxHeight = parseFloat(setHeight) + "px";

            if (_description.parentNode.clientHeight > setHeight) {
                var nice_bar = jQuery("#node_description").parent();
                nice_bar.niceScroll();
                nice_bar.niceScroll("div.nice-wrapper", {
                    cursorwidth: "8px"
                });
                jQuery("#node_description").parent().getNiceScroll().resize();
                if (document.querySelector(".nicescroll-rails")) {
                    document.querySelector(".nicescroll-rails").classList.remove("hideIt");
                }
            }
            else {
                if (document.querySelector(".nicescroll-rails")) {
                    document.querySelector(".nicescroll-rails").classList.add("hideIt");
                }
            }
            //end--height set
