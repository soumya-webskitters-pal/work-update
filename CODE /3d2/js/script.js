...
function selectElement(elName) {
  ...
  //add close button
  // console.log(scene.layers[0]);
  if (scene.layers[0].name === "UI_layer") {
    scene.layers[0].isEnabled = false;
    node =
      '<button class="close_btn"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 50" width="100px" height="100px"><path d="M 25 2 C 12.309534 2 2 12.309534 2 25 C 2 37.690466 12.309534 48 25 48 C 37.690466 48 48 37.690466 48 25 C 48 12.309534 37.690466 2 25 2 z M 25 4 C 36.609534 4 46 13.390466 46 25 C 46 36.609534 36.609534 46 25 46 C 13.390466 46 4 36.609534 4 25 C 4 13.390466 13.390466 4 25 4 z M 32.990234 15.986328 A 1.0001 1.0001 0 0 0 32.292969 16.292969 L 25 23.585938 L 17.707031 16.292969 A 1.0001 1.0001 0 0 0 16.990234 15.990234 A 1.0001 1.0001 0 0 0 16.292969 17.707031 L 23.585938 25 L 16.292969 32.292969 A 1.0001 1.0001 0 1 0 17.707031 33.707031 L 25 26.414062 L 32.292969 33.707031 A 1.0001 1.0001 0 1 0 33.707031 32.292969 L 26.414062 25 L 33.707031 17.707031 A 1.0001 1.0001 0 0 0 32.990234 15.986328 z"/></svg></button>';
  }

  //add title
  let add_title = '<h2 class="pr_title">' + elName + "</h2>";

  //add structure
  set_meterials.innerHTML = add_title + target_div.innerHTML.trim() + node;
  ...
  // open panel
  set_meterials.classList.remove("close");
  set_meterials.classList.add("open");
  gsap.to(set_meterials, {
    duration: 0.5,
    x: 0,
    pointerEvents: "all",
  });
  ...
  let close_btn = set_meterials.querySelector(".close_btn");
  close_btn.addEventListener("click", function () {
    set_meterials.classList.add("close");
    set_meterials.classList.remove("open");
    gsap.to(set_meterials, {
      duration: 0.5,
      x: 300,
      pointerEvents: "none",
      onComplete: () => {
        scene.layers[0].isEnabled = true;
        //blank parent div
        set_meterials.innerHTML = "";
      },
    });
...
//         console.log(
//           "scene pointer event - pickedMesh:",
      ...
      meshes.forEach(function (each_mesh, idx) {
        if (!each_mesh.name.includes("root")) {
          ...
          button.onPointerClickObservable.add((evt) => {
            ...
            // gsap.to(scene.activeCamera, {
            //   fov: 0.1,
            //   duration: 1,
            // });
            // console.log(meshes[2]);
            // camera.setTarget(meshes[2].position);
            // gsap.to(scene.activeCamera.position, {
            //   y: scene.activeCamera.position.y + 10,
            //   duration: 1,
            // });
            // camera.setTarget(new BABYLON.Vector3(-0.1, 0.12, 0));
