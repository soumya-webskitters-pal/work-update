...
function slider3d(container) {
  ...
  function createMesh() {
    ...
    const tick = (p) => {
      ...
      for (let i = 0; i < objs.length; i++) {
        ...
        for (const intersect of intersects) {
          if (intersect.object.link.length) {
            flag = 1;
            if (clicked == 1) {
             window.location.href=intersect.object.link;
            }
          }
      ...
      position += posY;
      position = gsap.utils.clamp(0, slideToShow, position);
      posY *= 0.9;
      // gsap.to(camera.position, {
      //   x: position + scrollSpeed * -0.005,
      // });
